...
const RPC_URLS = {
  ...
  base: process.env.BASE_RPC || 'https://base-rpc.publicnode.com',
  fantom: process.env.FANTOM_RPC || 'https://fantom-rpc.publicnode.com',
  gnosis: process.env.GNOSIS_RPC || 'https://gnosis-rpc.publicnode.com',
  ...
  goerli: process.env.GOERLI_RPC || 'https://ethereum-goerli-rpc.publicnode.com',
  amoy: process.env.AMOY_RPC || 'https://rpc-amoy.polygon.technology',
...
function getCached(key) {
  ...
  if (Date.now() - entry.ts > CACHE_TTL) { metadataCache.delete(key); return null; }
  return entry.data;
}
function setCache(key, data) {
  if (metadataCache.size > 5000) metadataCache.clear();
...
const IPFS_GATEWAYS = [
  'https://ipfs.gembaticket.com/ipfs/',
...
async function fetchFromGateway(url, timeoutMs) {
  ...
  try {
    ...
    if (text.startsWith('<!DOCTYPE') || text.startsWith('<html') || text.startsWith('<?xml')) return null;
    return JSON.parse(text);
  } catch { clearTimeout(timer); return null; }
}

async function fetchWithGatewayFallback(ipfsPath, timeoutMs) {
  const cached = getCached(ipfsPath);
  if (cached) return cached;

  // Race first 2 gateways, then try rest sequentially
  const fast = await Promise.any([
    fetchFromGateway(IPFS_GATEWAYS[0] + ipfsPath, timeoutMs || 8000),
    fetchFromGateway(IPFS_GATEWAYS[1] + ipfsPath, timeoutMs || 8000),
  ]).catch(() => null);

  if (fast) { setCache(ipfsPath, fast); return fast; }

  for (let i = 2; i < IPFS_GATEWAYS.length; i++) {
    const result = await fetchFromGateway(IPFS_GATEWAYS[i] + ipfsPath, timeoutMs || 8000);
    if (result) { setCache(ipfsPath, result); return result; }
  }
  return null;
}
...
async function fetchMetadataFromURI(uri, tokenId) {
  if (!uri) return null;
  const templated = uri.replace('{id}', String(tokenId).padStart(64, '0'));

  if (templated.startsWith('ipfs://')) {
    const ipfsPath = templated.slice(7);
    return fetchWithGatewayFallback(ipfsPath, 10000);
  }

  const resolved = resolveIPFS(templated);
  try {
    const controller = new AbortController();
...
async function enrichMissingMetadata(results, chainKey) {
  return Promise.all(results.map(async (nft) => {
    try {
      ...
      // If still missing metadata, fetch from tokenUri with multi-gateway
      if (nft.tokenUri && (!nft.animationUrl || !nft.image || nft.name === '#' + nft.tokenId)) {
        const meta = await fetchMetadataFromURI(nft.tokenUri, nft.tokenId);
        if (meta) {
...
function resolveIPFS(url) {
  ...
  if (url.startsWith('ipfs://')) {
    ...
    // CIDv1 (bafybei/bafkrei) -> public gateway (browser handles challenges)
    // CIDv0 (Qm) -> self-hosted node (fast, local content)
    if (path.startsWith('bafybei') || path.startsWith('bafkrei')) {
      return 'https://nftstorage.link/ipfs/' + path;
    }
    return 'https://ipfs.gembaticket.com/ipfs/' + path;
  }
